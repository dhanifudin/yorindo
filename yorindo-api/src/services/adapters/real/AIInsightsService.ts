...
const CACHE_TTL = 5_000 // 5 seconds - shorter for development
...
export class AIInsightsService implements IYoriMindService {
  async analyze(snapshot: EventSnapshot): Promise<YoriMindResult> {
    ...
    console.log('[AIInsights] Settings loaded:', {
      ...
      hasBaseUrl: !!settings.AI_BASE_URL,
    })

    // Check if AI is disabled or using mock
    ...
    if (settings.AI_PROVIDER === 'mock' || !settings.AI_API_KEY) {
      console.log('[AIInsights] Falling back to mock (provider=%s, hasKey=%s)', settings.AI_PROVIDER, !!settings.AI_API_KEY)
      return this.getMockAnalysis(snapshot)
    }
    ...
    console.log('[AIInsights] Calling real AI: provider=%s, model=%s, baseUrl=%s', settings.AI_PROVIDER, model, baseUrl)
  ...
  private async getMockAnalysis(snapshot: EventSnapshot): Promise<YoriMindResult> {
    ...
    return {
      ...
      recommendations: [
        {
          action: 'Kirim reminder H-3 dan H-1 ke peserta yang sudah disetujui',
          priority: 'high' as const,
          impact: 'Meningkatkan kehadiran hingga 15-20%',
        },
        {
          action: 'Evaluasi kriteria approval untuk meningkatkan konversi',
          priority: 'medium' as const,
          impact: 'Memperluas basis peserta yang relevan',
        },
        {
          action: 'Buat grup WhatsApp atau Telegram untuk peserta',
          priority: 'low' as const,
          impact: 'Meningkatkan engagement dan sense of community',
        },
      ],
