...
Admin can access event performance insights and reporting from within the Event Pipeline Hub (Laporan tab); vendors can access report flows through the current runtime vendor-report path while richer analytics/reporting behavior continues to evolve.

> **Updated 2026-03-28** — Meeting: YoriMind has no dedicated top-level menu. It is accessed exclusively via the Laporan tab inside the Event Pipeline Hub (`/app/events/:id/report`), making AI insights seamlessly contextual to the event being viewed. This was already implemented structurally in Story 4.12; this epic's scope remains unchanged — only the navigation entry point is clarified.

> **Current implementation note (2026-03-30):** The current runtime supports the core report/YoriMind/vendormagiclink contract surface, but the full richer analytics/dashboard/filter/export behavior described below is not fully implemented everywhere yet.
> **Phase 1 (FE):** Post-event report page, vendor magic link landing page + DPA acceptance gate, analytics/report widgets, YoriMind panel, and download controls are represented in the current FE/MSW flow.
> **Phase 2 (BE):** Current runtime includes `GET /api/events/:id/report`, `POST /api/events/:id/report/regenerate`, `GET /api/events/:id/report/download`, `POST /api/events/:id/vendor-link`, `GET /api/events/:id/yorimind`, and vendor-report token endpoints. Deeper export/rendering details remain follow-up work.
...
**Given** an admin calls `POST /api/events/:id/report/regenerate`,
...
**Given** an admin has attached a vendor sponsor to the event through `/api/vendors` and `/api/events/:id/sponsors`,
**When** `POST /api/events/:id/vendor-link` is called,
**Then** a time-limited signed URL is generated (max 7-day expiry) and delivered to the sponsor's configured `contact_email` from the linked vendor record (FR41, FR45, NFR-S12)
...
**Given** I am on the event report tab (`/app/events/:id/report`),
