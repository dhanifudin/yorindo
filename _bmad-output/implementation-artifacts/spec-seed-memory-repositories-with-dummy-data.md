...
**Execution:**
- [x] `src/repositories/memory/_seeds.ts` -- CREATE: export `SEED_EVENT_IDS` (12 CUID2 literals), `SEED_CONTACT_IDS` (120 CUID2 literals), `SEED_USER_IDS` (5 CUID2 literals keyed by role), `SEED_VENDOR_IDS` (3 CUID2 literals), `SEED_UPLOAD_IDS` (3 CUID2 literals), `PASSWORD123_HASH` (bcrypt.hashSync result), `INDONESIAN_INDUSTRIES` and `INDONESIAN_JOB_TITLES` lookup arrays with id+name — all IDs are hand-crafted valid CUID2 strings (24 chars, lowercase alphanumeric, leading letter, no hyphens)
- [x] `src/repositories/memory/UserRepository.ts` -- rewrite `_seed()` to add 5 users: `super_admin` (super@yorindo.id), `event_admin` (admin@yorindo.id), `staff` (staff@yorindo.id), `vendor_client` (vendor@yorindo.id), `participant` (participant@yorindo.id) — all using `PASSWORD123_HASH` from _seeds
