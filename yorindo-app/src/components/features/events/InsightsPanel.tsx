...
export function InsightsPanel({ eventId }: InsightsPanelProps) {
  const queryClient = useQueryClient()
  const [isExpanded, setIsExpanded] = useState(false)

  const { data, isLoading, isFetching } = useQuery<InsightsResult>({
    queryKey: ['insights', eventId],
    queryFn: () => fetch(`/api/events/${eventId}/insights`).then((r) => r.json()),
    enabled: isExpanded,
    staleTime: 24 * 60 * 60 * 1000,
  })

  const handleRefresh = async () => {
    await fetch(`/api/events/${eventId}/insights/cache`, { method: 'DELETE' })
    queryClient.invalidateQueries({ queryKey: ['insights', eventId] })
    queryClient.refetchQueries({ queryKey: ['insights', eventId] })
  }
